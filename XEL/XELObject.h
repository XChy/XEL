...
public:
	MappedVariable();
	MappedVariable(XHashMap<XString, Variant>* map,const XString& variableName);

	Variant invoke(const XString& funcName, const std::vector<Variant>& params, const XELObjectWrapper& thisWrapper);
	bool hasMemberFunction(const XString& funcName, int paramNum) const;
	Variant member(const XString& memberName);
	bool hasMember(const XString& memberName) const;
	XString toString() const;
