...
MappedVariable::MappedVariable(XHashMap<XString, Variant>* map, const XString& variableName)
	: _map(map), _variableName(variableName)
{
}

Variant MappedVariable::invoke(const XString& funcName, const std::vector<Variant>& params, const XELObjectWrapper& thisWrapper)
{
	if (variable().type() == VariantType::Object)
		return variable().objectValue().invoke(funcName, params);
	else
		throw XELError("Try to invoke the member function of a non-object variant");
}

bool MappedVariable::hasMemberFunction(const XString& funcName, int paramNum) const
{
	if (variable().type() == VariantType::Object)
...
XString MappedVariable::toString() const
{
	return variable().toString();
}

XString MappedVariable::type() const
{
	return "MappedVariable";
}

Variant MappedVariable::variable() const
{
	return _map->value(_variableName);
}

void MappedVariable::setMap(XHashMap<XString, Variant>* map)
{
	_map = map;
}

XHashMap<XString, Variant>* MappedVariable::map()
{
	return _map;
}

XString MappedVariable::variableName() const
{
